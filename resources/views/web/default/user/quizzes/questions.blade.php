            ...
            <div class="ucp-section-box">
                <div class="header back-red headst">
                    ...
                    <div>
                        ...
                        <button type="button" class="btn btn-primary btn-round " id="addDragDrop">Drag Drop</button>
                ...
                @if($quiz->mega_quiz === 1)
                        ...
                        <?php $i = $allQue->perPage() * ($allQue->currentPage() -1); ?>
                            @foreach ($allQue as $question)
                                <div class="question-item">
                                    ...
                                    <div>
                                        <button data-id="{{ $question->question_id }}" data-type="{{ $question->type }}" class="gray-s btn-transparent btn-question-edit" data-toggle="tooltip" title="{{ trans('main.edit_question') }}">
                                            <span class="crticon mdi mdi-lead-pencil"></span>
                                        </button>
                                        <button data-id="{{ $question->question_id }}" class="btn-transparent btn-delete-question" data-toggle="tooltip" title="{{ trans('main.delete') }}"><span class="crticon mdi mdi-delete-forever"></span></button>
                                    </div>
                                </div>
                            @endforeach
                        </div>
                        <div class="card-footer text-center">
{{--                            {!! $quiz->appends($_GET)->links('pagination.default') !!}--}}
                            {!! $allQue->links('pagination.default') !!}
                        </div>
                    @else
    ...
    <div id="dragModal" class="modal fade" role="dialog">
        <div class="modal-dialog zinun">
            <!-- Modal content-->
            <div class="modal-content">
                <div class="modal-header">
                    <h3>Drag & Drop Question</h3>
                </div>
                <div class="modal-body modst">
                    @include(getTemplate() .'.user.quizzes.drag_question_form')
                </div>
                <div class="modal-footer">
                    <button type="button" id="dragAnswerSubmit" class="btn btn-custom">{{ trans('main.save') }}</button>
                </div>
            </div>
        </div>
    </div>
...
@section('script')
    ...
    <script>
        ...
        $('#dragAnswerSubmit').on('click',function () {
            $('#dragAnswer').submit();
        });
...
//            var dragModal = $('#dragModal');
        ...
        $('body').on('click', '#addDragDrop', function (e) {
            e.preventDefault();
            var modal = $('#dragModal');
            ...
            answerBox.each(function (index, box) {
                if (!$(box).hasClass('main-row-answers')) {
                    $(box).remove();
                }
            });
            modal.modal('show');
        ...
        $('body').on('click', '.add-btnd', function (e) {
            e.preventDefault();
            var mainRow = $('.main-row-answers');
            var copy = mainRow.clone();
            copy.removeClass('main-row-answers');
            // var random_id = randomString();
            // copy.find('.lfm-btn').attr('data-input', random_id);
            // copy.find('.lfm-input').attr('id', random_id);
            var copyHtml = copy.prop('innerHTML');
