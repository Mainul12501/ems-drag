                ...
                <div class="tab-content">
                    <div id="main" class="tab-pane active">
                        <form action="/admin/user/edit/{{$user->id}}" class="form-horizontal form-bordered" method="post">
                            ...
                            ?>
                            <div class="form-group">
                                <label class="col-md-3 control-label">Student Role</label>
                                <div class="col-md-6">
                                    <select name="student_role_id" class="form-control populate">
                                        @foreach($roles as $role)
                                            <option value="{{ $role->id }}" {{ $user->student_role_id == $role->id ? 'selected="selected"' : '' }}>{{ $role->display_name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
