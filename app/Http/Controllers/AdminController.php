...
class AdminController extends Controller
{
    ...
    public function userEdit($id, Request $request)
    {
        ...
        if ($request->vendor == 1)
        {
            $data['student_role_id'] = 4;
        } else {
            $data['student_role_id'] = $request->student_role_id;
        }
//        $uUser->update($request->all());
    ...
    public function QuizzesList()
    {
        ...
        $quizzes = Quiz::with(['content','user', 'questions', 'QuizResults'])->paginate(15);
...
//    mainul
    public function userRolesLists()
    {
        $roles  = StudentRole::all();
        return view('admin.student-role.list',['roles'=>$roles]);
    }
    public function userRolesedit($id)
    {
        $studentRole   = StudentRole::findOrFail($id);
        $roles  = StudentRole::all();
        return view('admin.student-role.list',['roles'=>$roles,'studentRole'=>$studentRole]);
    }
    public function userRolesdel($id)
    {
        $del = StudentRole::findOrFail($id);
        $del->delete();
        return \redirect()->back();
    }
    public function userRolestore(Request $request)
    {
        StudentRole::updateOrCreate(['id'=>$request->role_id],[
            'name'  => $request->name,
