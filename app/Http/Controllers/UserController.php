...
use App\Models\StudentQuizType;
...
class UserController extends Controller
{
    ...
    public function dashboard()
    {
        ...
        $userNotification = Notification::where(function ($q) {
            ...
            // $date = $currentDate->subDays($currentDate->dayOfWeek + 1);
            // $date = \Carbon\Carbon::now()->subDays(7);
            // $date=date('Y-m-d', strtotime("-7 day", date('Y-m-d')));
            // $mark = QuizResult::where('created_at','>=',$date)
            $mark = QuizResult::where('student_id', $user->id)
            ->orderBy('user_grade','DESC')
            ->get();
            $countans = QuizResult::where('student_id', $user->id)
            ->count('results');
            // dd($countans);
            $search='people';
            $question = QuizzesQuestion::select(QuizzesQuestion::raw('count(*) as count'))
            ...
            // dd($question);
            $search1='process';
            $question1 = QuizzesQuestion::select(QuizzesQuestion::raw('count(*) as count'))
            ->Where('category', 'like', '%' . $search1 . '%')
            // ->first()
            ->sum('grade');

            // dd($question1);
            $total=$question+$question1;
            // dd($total);
            // $count=QuizResult::where('quiz_id',$question->id)
            // ->with('question_id')
            // ->first();
            // dd($count);
            // graph
            ...
            //              ->with(['questions', 'questionsAnswers', 'QuizResults' => function ($query) {
            //                  $query->where('result_id',$result->id);
            //                  $query->where('question_id', $question->id);
            //                  $query->where('category','process');
            //                  $query->where('correct','0');
            //                  $query->with('student');
            //              }])->count();
            //              dd($quizzes);
            $get_responses = DB::table('quizzes_questions_answers')
                            ->where('user_id', $user->id)
                            ->groupBy('question_id','title')
                            ->where('correct','0')
        ...
        // $quiz_answer = QuizResult::where('id', $result_id)
        //     ->where('student_id', $user->id)
        //     ->with(['quiz' => function ($query) {
        //         $query->with(['questionsAnswers','questionsGradeSum']);
        //     }])
        // ->first();
        ...
        // ->where('user_id', $user->id)
        // ->first();
        // $total_type = QuizzesQuestion::where('id', $quiz_id)->where('category', 'people')->sum('grade');
        // dd($total_type);
        ...
        for ($i = 20; $i >= 1; $i--) {
            ...
            //     ->count();
                // $sellDay  =  QuizResult::where('student_id', $user->id)->whereStatus('pass')->count();
                // dd($sellDay);
        // $dayRegister = QuizResult::where('student_id', $user->id)->where('created_at', '>', strtotime('-' . get_option('chart_day_count', 10) . ' day') + 12600)->get();
        $dayRegister = DB::table('quizzes_results')
        ->where('student_id', $user->id)
        ->where("created_at",">", Carbon::now()->subDays(7))
        ->get();
        // dd($dayRegister);

       $totalmark=DB::table('quizzes_results')
       ->where('student_id', $user->id)
    //    ->where('user_grade')
       ->sum('user_grade');
       $subtotal=$totalmark*6;
    //    dd($subtotal);
    //    ->get();
    //    dd($totalmark);
    // $quiz = Quiz::where('id', $quiz_id)->first();
    // $totalgrade=DB::table('quizzes_questions')
    // ->where('quiz_id', $quiz->id)
    // ->sum('grade');
    // dd($totalgrade);

        // dd($dayRegister);
                // $sellDay  =  QuizResult::where('student_id', $user->id)->where('user_grade')->get();
            ...
            //     ->where('created_at', '<', $timestamp + 86400)
            //     ->sum('income');
                $incomeDay = QuizResult::where('student_id', $user->id)->whereStatus('fail')->count();
        }
        $incomeDay = QuizResult::where('student_id', $user->id)->whereStatus('waiting')->count();
    ...
    ## Simulator Section ##
//    new edit function start
public function simForm(Request $request)
{
    $checkMethod = preg_replace('/[0-9]+/', '', $request->sim_qty);
    if ($checkMethod == 'sim')
    {
//            $this->simulatedQuiz($request);
        //        que, time, pass mark area
        $localData  = [];
        if ($request->sim_qty == 'sim50')
        {
            $localData['sim_qty']   = 50;
            $localData['sim_time']   = 60;
//            $localData['sim_pass_mark']   = 61; //raw
            $localData['sim_pass_mark']   = 30; //percent
        } elseif ($request->sim_qty == 'sim100')
        {
            $localData['sim_qty']   = 100;
            $localData['sim_time']   = 120;
            $localData['sim_pass_mark']   = 61;
        } elseif ($request->sim_qty == 'sim200')
        {
            $localData['sim_qty']   = 200;
            $localData['sim_time']   = 240;
//            $localData['sim_pass_mark']   = 61; //raw
            $localData['sim_pass_mark']   = 122; //percent
        }
//        que, time, pass mark area ends
//        random questions start
//        $questions  = QuizzesQuestion::with('questionsAnswers')->get();
        $questions  = MegaQuiz::with('questionsAnswers')->get();
//        return $questions;
        if (count($questions)<$localData['sim_qty'])
        {
            $randomQuestions    = $questions;
        } else{
            $randomQuestions    = $questions->random($localData['sim_qty']);

        }
        //        random questions ends
//        Quiz result start

        $user = auth()->user();
        $quiz   = Quiz::where('mega_quiz', 1)->first();
        ...
        //        Quiz result end
        return view(getTemplate() . '.user.simulator.start',[
            'localData'  => $localData,
    ...
    } elseif ($checkMethod == 'pra')
    {

        if ($request->sim_qty == 'pra10')
        {
            $que_qty    = 10;
        } elseif ($request->sim_qty == 'pra25')
        {
            $que_qty    = 25;
        } elseif ($request->sim_qty == 'pra50')
        {
            $que_qty    = 50;
        } elseif ($request->sim_qty == 'pra100')
        {
            $que_qty    = 100;
        }
//            $questions  = MegaQuiz::where('category','like','%'.$request->people.'%')->with('questionsAnswers')->get();
        $questions  = MegaQuiz::where('category', 'like', '%' . $request->people . '%')
        ->orWhere('category', 'LIKE', '%' .$request->people. '%')
        ->orWhere('category', 'LIKE', '%' .$request->process. '%')
        ->orWhere('category', 'LIKE', '%' .$request->enviornment. '%')
        ->orWhere('category', 'LIKE', '%' .$request->predictive. '%')
        ->orWhere('category', 'LIKE', '%' .$request->agile. '%')
        ->orWhere('category', 'LIKE', '%' .$request->high_team. '%')
        ->orWhere('category', 'LIKE', '%' .$request->starting_project. '%')
        ->orWhere('category', 'LIKE', '%' .$request->doing_work. '%')
        ->orWhere('category', 'LIKE', '%' .$request->keep_track. '%')
        ->orWhere('category', 'LIKE', '%' .$request->keep_mind. '%')
            ->with('questionsAnswers')->get();

//        return $questions;
        if (count($questions)<$que_qty)
        {
            $randomQuestions    = $questions;
        } else{
            $randomQuestions    = $questions->random($que_qty);
        }
        return view(getTemplate() . '.user.simulator.practice.start',[
            'que_qty'  => $que_qty,
            'randomQuestions'   => $randomQuestions,
        ]);
    }
}
public function getAns($questionId)
{
    $ans    = QuizzesQuestionsAnswer::where('question_id', $questionId)->where('correct', 1)->first();
    // return json_encode($ans);
    $que    = MegaQuiz::where('question_id', $questionId)->select('id','description')->first();
        $result = [
            'answer'   => $ans,
            'question'  => $que
        ];
    ...
    public function SimulatorList(){
        // $user = auth()->user();
        // $questions=QuizzesQuestion::whereNotIn('id', \DB::table("skips")
        //                        ->select("question_id")
        //                        ->where("user_id", \Auth::id()))
        //                     //    ->toArray()
        //                        ->random();
        // $skip = Skips::where('user_id', $user_id)->select('question_id')->toArray();
        // $questions = QuizzesQuestion::select("question_id")->whereNotIn('question_id', $skip)->toArray();
        // $questions= QuizzesQuestion::doesntHave('skips')->random();
        $user = auth()->user();

        // $quiz = Quiz::where('id', $quiz_id)
        //     ->where('user_id', $user->id)F
        //     ->first();
        $search='people';
            // $search='keeping the team on track';
            // $search='creating a high performance team';
            $count = QuizzesQuestion::Where('category', 'like', '%' . $search . '%')
                    //  ->first()
                    //  ->count;
                    ->pluck('title')->toArray();
                    //  dd($count);
        ...
        return view(getTemplate() . '.user.simulator.list', $user);
        // return view('user.simulator.list',compact('user'));

             }
    }
 public function SimulatorNew(){
    $user = auth()->user();
    return view(getTemplate() . '.user.simulator.simulatorDuplicate', $user);
 }
    public function SimulatorAnswer(){
        $user = auth()->user();
    ...
    //     return view(getTemplate() . '.user.simulator.category',compact('parentCategories','user'));
    //     // return view('user.simulator.list',compact('user'));
    ...
    #######drag-drop###########
     public function DragDrop(){
        $user = auth()->user();
        // return view('web.default.user.quizzes.dragdrop',compact('user'));
        return view('.user.simulator.dragdrop',$user);
     }
    ...
    return view(getTemplate() . '.user.simulator.category',compact('categories','user'));
    ...
    public function userProfileView(Request $request,$id)
    {
        ...
        // $question= QuizResult::where('student_id', $id)
        // ->get()->pluck('results');
        ...
        // dd($end_week);
        // $last_row=QuizResult::whereBetween('created_at', [$start_week, $end_week])->get(['user_grade','created_at']);
        // dd($last_row);
        // $last_row=DB::table('quizzes_results')->where('student_id', $id)->orderBy('id', 'DESC')->first();
        // dd($last_row);
        // $stat= DB::table('quizzes_results')->where('student_id', $id)->latest('status')->first();
        $w1= DB::table('quizzes_results')->where('student_id', $id)->count();
        // dd($w1);
        // dd($stat);
        ...
        $last = DB::table('quizzes_results')->where('student_id', $id)->get()->last();
        // $last3=$last/
        // dd($last);
        // $total=DB::table('quizzes_results')->where('student_id', $id)->where('quiz_id', $id);
     ...
     //    ->where('user_grade')
        ...
        $subtotal=$totalmark*1+3;

        $sub_total=$totalmark*1-9;
        $sub_total1=$totalmark*1-10;
        $sub_total2=$totalmark*1-2;
        $sub_total3=$totalmark*1-9;
        $sub_total4=$totalmark*1-6;


        $subtotal1=$totalmark*1+4;
        $subtotal2=$totalmark*1-9;
        $subtotal3=$totalmark*1+3;
        $subtotal4=$totalmark*1-8;
...
//        $quizzesQuery = Quiz::query();
        ...
        if ($user->vendor) {
            ...
            $typeTable  = StudentQuizType::where('student_role_id', $user->student_role_id)->get();

            foreach ($typeTable as $type)
            {
                $xx[]    = Quiz::where('id', $type->quiz_id)->where('status', 'active')
                    ->with(['questionsGradeSum', 'content'])->first();
            }
...
//return $quizzes;
                ###############
            // $quiz_result = $QuizResult->where('total' , 'active')
            // ->with(['questionGradeSum' , 'content'])
            // ->get();


                ##########
                $quizzes = array_filter($xx, function($a) {return $a !== null;});
...
//        save student quiz types mainul
        foreach ($data['student_type'] as $type)
        {
            StudentQuizType::create([
                'quiz_id'   => $quiz->id,
                'user_id'   => $user->id,
                'student_role_id'   => $type,
            ]);
        }
//        new code ends
    ...
    public function QuizzesEdit($quiz_id)
    {
        ...
        $quiz = Quiz::where('id', $quiz_id)
            ->where('user_id', $user->id)
            ->first();
            $studentRoles    = StudentQuizType::where('quiz_id', $quiz_id)->with('student')->get();
        if (!empty($quiz)) {
            $data = [
                'user' => $user,
                'quiz' => $quiz,
    ...
    public function QuizzesUpdate(Request $request, $quiz_id)
    {
        ...
        if (!empty($quiz)) {
            ...
            //        save student quiz types mainul
            $existTypes = StudentQuizType::where('quiz_id', $quiz->id)->get();
            if (count($existTypes)>0)
            {
                foreach ($existTypes as $del)
                {
                    $del->delete();
                }
            }
    ...
    public function QuizzesDelete($quiz_id)
    {
        ...
        $quiz = Quiz::where('id', $quiz_id)
            ...
            ->first();
//        mainul
        $studentTypes   = StudentQuizType::where('quiz_id', $quiz->id)->get();
...
//        new code ends
    ...
    public function QuizzesQuestions($quiz_id)
    {
        ...
        if ($quiz->mega_quiz === 1)
        {
//            $allQue = QuizzesQuestion::select('id','user_id','quiz_id','title','type')->paginate(15);
//            $allQue = QuizzesQuestion::paginate(15);


//            $allQuestions = QuizzesQuestion::all();
//            foreach ($allQuestions as $meagQuestion)
//            {
//                $existRow   = MegaQuiz::where('title',$meagQuestion->title)->where('quiz_id', $quiz->id)->first();
//                if (!$existRow)
//                {
//                    $mega   = new MegaQuiz();
//                    $mega->user_id  = $meagQuestion->user_id;
//                    $mega->quiz_id  = $quiz->id;
//                    $mega->question_id  = $meagQuestion->id;
//                    $mega->title  = $meagQuestion->title;
//                    $mega->image  = $meagQuestion->image;
//                    $mega->grade  = $meagQuestion->grade;
//                    $mega->type  = $meagQuestion->type;
//                    $mega->category  = $meagQuestion->category;
//                    $mega->description  = $meagQuestion->description;
//                    $mega->save();
//                }
//            }
            $allQue = MegaQuiz::paginate(15);
...
//            test end
        ...
        if (!empty($quiz)) {
            ...
            return view(getTemplate() . '.user.quizzes.questions', ['quiz' => $quiz,'allQue'    => $allQue,]);
...
//        if ($user->student_role_id)
        $studentQuizTypes   = StudentQuizType::where('quiz_id', $quiz_id)->get();
        foreach ($studentQuizTypes as $studentQuizType)
        {
            if ($studentQuizType->student_role_id == $user->student_role_id)
            {
                $quiz = Quiz::where('id', $quiz_id)
                ...
                if ($quiz) {
                    ...
                    // $answer = QuizzesQuestionsAnswer::paginate(8);
                    // $quiz=Quiz::paginate(8);
        ...
        return \redirect()->back()->with('error', 'You can not participate in this quiz.');
    }

    public function megaQuizzesStoreResult(Request $request, $quiz_id)
    {
//        keep data on session
                    Session::put('sim_qty', $request->sim_qty);
                    Session::put('sim_pass_mark', $request->sim_pass_mark);
                    Session::put('sim_total_mark', $request->sim_qty);
        $user = auth()->user();
        $quiz = Quiz::where('id', $quiz_id)->first();


        $questionIds    = $request->question_ids; //new code
        session()->put('mega_quiz_questions', $questionIds); //new code
        ...
        if ($quiz) {
            ...
            foreach ($results as $question_id => $result)
            {
                ...
                {
                    $ansIdArray[] = $result['answer'];
                }
            }
            session()->put('mega_quiz_answers', $ansIdArray);
//            new code ends
            ...
            if (!empty($quiz_result_id)) {
                ...
                if (!empty($quiz_result)) {
                    $pass_mark = $request->sim_pass_mark;
//                    $total_mark = $request->sim_qty;
                    $total_mark = 0;
                    $status = '';
...
//                            $question = QuizzesQuestion::where('id', $question_id)->where('quiz_id', $quiz->id)->first();
                            $question = MegaQuiz::where('question_id', $question_id)->where('quiz_id', $quiz->id)->first();
...
//                                    ->where('question_id', $question->id)
                                ...
                                if ($question->type == 'descriptive') {
                                    $status = 'waiting';
                                    // $total_mark += (int)$question->grade; ##grade
                                    // $explanation += (int)
                                }
                            }
                        }
                    ...
                    return redirect('/user/quizzes/results/' . $quiz_result->id.'/final');
                }
            }
        }
        abort(404); // related twitted https triort
    }
    ...
    public function QuizzesStoreResult(Request $request, $quiz_id)
    {
        ...
        if ($quiz) {
            ...
            session()->put('normal_quiz_questions', $questionIds); //new code
            $ansIdArray = [];
            foreach ($results as $question_id => $result)
            {
                if (!empty($result['answer']))
            ...
            session()->put('normal_quiz_answers', $ansIdArray);
//            new code ends
    ...
    public function megaStudentQuizzesResults(Request $request,$result_id){

        $queIds = session()->get('mega_quiz_questions');
        foreach ($queIds as $queId)
        {
            $ques[]  = MegaQuiz::where('question_id',$queId)->with('questionsAnswers')->first();
        }
        $ansIds = session()->get('mega_quiz_answers');
        foreach ($ansIds as $ansid)
        {
            $Answers[]  = QuizzesQuestionsAnswer::where('id', $ansid)->first();
//            $providedAnswers[]  = MegaQuiz::where('id', $ansid)->first();
        }
...
//                $query->with(['questions', 'questionsGradeSum']);
                $query->with(['megaQuiz', 'megaQuestionsGradeSum']);
        ...
        if ($quiz_result) {
            ...
            $quizResult11 = QuizResult::findOrFail($result_id);

            foreach ($quiz->questions as $question) {
                $question->pluck('category')->first();

            }
            $question_review=$quiz->questions->sum('grade');
            ...
            return view(getTemplate() . '.user.simulator.student_results', $data);
        }
        abort(404);
    }

    public function StudentQuizzesResults(Request $request,$result_id){

        $queIds = session()->get('normal_quiz_questions');
        foreach ($queIds as $queId)
        {
            $ques[]  = QuizzesQuestion::where('id',$queId)->with('questionsAnswers')->first();
        }
        $ansIds = session()->get('normal_quiz_answers');
        foreach ($ansIds as $ansid)
        {
            $providedAnswers[]  = QuizzesQuestionsAnswer::where('id', $ansid)->first();
        }
        ...
        //show all question title
            // $question_review=$quiz->questions->pluck('title');
            $question_review=$quiz->questions->sum('grade');
            // $question_review1=(($question_review*80)/100)
            // dd($question_review1);
    ...
    public function QuizzesQuestionsEdit(Request $request, $question_id)
    {
        ...
        $catArray  = explode(', ',$question->category);
        $index  = count($catArray)-1;
    ...
    #alpha testing beta gamma testing $html
...
// return $request->all();
        ...
        $question = QuizzesQuestion::where('id', $question_id)
            ->where('user_id', $user->id)
            ->first();
        $megaQuestion   = MegaQuiz::where('question_id', $question_id)->where('user_id', $user->id)->first();
        if (!empty($megaQuestion))
        {
...
//                 'category' => $data['category'],
                'category' =>implode(", ",$datax['category']),
            ];
            $megaQuestion->update($megaData);
        }
