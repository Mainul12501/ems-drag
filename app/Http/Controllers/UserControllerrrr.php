...
//        $quizzesQuery = Quiz::query();
        ...
        if ($user->vendor) {
            ...
            {
                $quizzes[]    = Quiz::where('id', $type->quiz_id)->where('status', 'active')
...
//                $query->with(['questions', 'questionsGradeSum']);
    ...
    public function StudentQuizzesResults(Request $request,$result_id){
        ...
        ///
//         $results = DB::table('quizzes_questions')->where('user_id', $user->id)->get();
//         $quiz_result_id =  DB::table('quizzes_results')->where('student_id', $user->id)->get();
// $quiz_result = QuizResult::where('id', $quiz_result_id)
//                 ->where('student_id', $user->id)
//                 ->first();
...
//         dd($quiz_result);
        ...
        $quiz_result = QuizResult::where('id', $result_id)
            ...
            /////
            // $quiz_resulttt = QuizResult::where('id', $result_id)
            // ->where('student_id', $user->id)
            // ->with(['questions' => function ($query) {
            //     $query->with(['quiz']);
            //     $query->where('categoty','people');
            // }])
            // ->first();
            ...
            //             ->where('user_id', $user->id)
            //             ->first();
            //         if (!empty($question) and $question->type == 'multiple') {
            //             $item = [
            //                 'question_id' => $question->id,
            //                 'question' => $question->title,
            //                 'question_grade' => $question->grade,
            ...
            // $articles11 =DB::table('quizzes_questions')
            // ->join('quizzes_questions_answers', 'quizzes_questions.id', '=', 'quizzes_questions_answers.id')
            // // ->join('users','user.id','=','articles.user_id','quizzes_questions_answers')
            // // ->join('users', 'users.id', '=', 'articles.user_id')
            // // ->join('quizzes_questions_answers','quizzes_questions_id')
            // ->select('quizzes_questions.id','quizzes_questions.title')
            // ->get();
            // dd($articles11);
            // $quiz_result11 = QuizResult::where('id', $result_id)
            // $quiz_answers = QuizResult::where('id','$result_id)
            // $quiz_answer = Quizzes
            // ->where('student_id', $user->id)
            // ->with(['quiz' => function ($query) {
            //     $query->with(['questions', 'questionsGradeSum']);
            // }])
            // ->first();
            // $result11=QuizResult::where('id', $result_id)
            // ->where('student_id', $user->id)
            // -> with(QuizzesQuestion::pluck('title'));
            // dd($result11);

//fetching and grouping same answers for particular question
        //     $get_responses = DB::table('quizzes_questions_answers')
        //                     ->groupBy('question_id','title')
        //                     ->select('title', DB::raw('COUNT(correct) as selected'))
        //                     ->pluck('selected', 'title');

        //    dd($get_responses);

        // $professionalism_Q1 =QuizResult::where('id', $result_id)
        // ->where('student_id', $user->id)
        // ->with(['quiz' => function ($query) {
        //     $query->with(['questions', 'questionsGradeSum']);
    ...
    public function QuizzesQuestionsUpdate(Request $request, $question_id)
    {
        ...
        $question = QuizzesQuestion::where('id', $question_id)
            ->where('user_id', $user->id)
            ->first();
        $megaQuestion   = MegaQuiz::where('question_id')->where('user_id', $user->id)->first();
